...
import Participation_Coefficient_Simple as MPC
...
default_params = { # 创建一个算法需要用到的参数字典
    ...
    "prop_pos": 0.7,                # Proportion of edges to remove nad use as positive samples
    "prop_neg": 0.3,                # Number of non-edges to use as negative samples
    "Reflash_test_data": False        #  (as a proportion of existing edges, same as prop_pos)
}

class Mergeing_vec_N2V:
    def __init__(self, path, sampling_p, p, q, num_walks, walk_length, num_partitions, dimensions, workers, ff, C):
        self.path = path
    ...
    def run(self):
        ...
        for steps in range(alg_num):
            ...
            for train_inx, test_inx in partitioner.split(edges_all, edge_labels_all):
                ...
                # Test classifier
                NMI.append(metrics.scorer.normalized_mutual_info_scorer(clf, edge_features_test, labels_test))
            ...
            print("%.4f"%(sum(auc_test)/len(auc_test)))
