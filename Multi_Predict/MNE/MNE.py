...
def train_deepwalk_embedding(walks, iteration=None):
    if iteration is None:
        iteration = 100
    model = Word2Vec(walks, size=100, window=5, min_count=0, sg=1, workers=4, iter=iteration)
    return model
...
def train_embedding(current_embedding, walks, layer_id, iter=10, info_size=10, base_weight=1):
    ...
    new_model = MNE(training_data, size=100, window=5, min_count=0, sg=1, workers=4, iter=iter, small_size=info_size, initial_embedding=initial_embedding, base_weight=base_weight)
